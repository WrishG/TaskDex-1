...
const style = {
  ...
  button: "px-8 py-4 rounded-xl font-bold transition-all duration-300 shadow-lg transform hover:scale-105 text-lg",
};
    ...
    <div className="flex flex-col items-center min-h-screen p-4 bg-[#1a1a1a] text-white animate-fadeIn">
      <div className={style.card + " max-w-2xl w-full mt-8"}>
        ...
        <div className="text-center mb-8">
          <h2 className="text-4xl font-bold text-white mb-2">Welcome Back, {trainerName}!</h2>
          <p className="text-gray-400">
            Your partner, <span className="font-semibold text-green-400">{firstPokemon}</span>, is ready to focus.
          </p>
        </div>
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-lg font-semibold text-white mb-3">Select Focus Type:</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-400 mb-2">Study Time (minutes):</label>
              <input
                type="number"
                value={studyTime}
                onChange={(e) => setStudyTime(Math.max(1, parseInt(e.target.value) || 0))}
                className="w-full p-3 rounded-lg bg-gray-700 border-2 border-gray-600 text-white focus:border-red-500 focus:ring-2 focus:ring-red-500 text-lg font-semibold placeholder-gray-400"
                min="1"
              />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-400 mb-2">Rest Time (minutes):</label>
              <input
                type="number"
                value={restTime}
